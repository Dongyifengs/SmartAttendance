  ...
  <div class="dev-home-page">
    ...
    >
      ...
      <div v-if="oneCard.billList.value.length" class="bill-table">
        <el-table :data="oneCard.billList.value" size="small" style="width: 100%">
          <el-table-column align="center" label="时间" min-width="35%" prop="trade_time" />
          <el-table-column align="center" label="说明" min-width="33%" prop="desc" />
          <el-table-column
    ...
    >
      <div class="qr-container">
        <img :src="paymentQR.qrCodeBase64.value" alt="支付二维码" class="qr-code" />
        <div class="qr-countdown">二维码 {{ paymentQR.refreshCountdown.value }} 秒后自动刷新</div>
      </div>
      <div class="user-details">
        {{ oneCard.userClass.value }} | {{ oneCard.userName.value }} | {{ oneCard.userId.value }}
      </div>
      <div class="qr-hint">请使用校园一卡通App扫码支付</div>
      ...
      <div class="air-conditioned-content">
        <el-form label-width="100px">
          ...
          <el-form-item label="空调余额">
            <el-text v-if="airConditioning.balance.value" size="large" type="primary">
...
<script lang="ts" setup>
  ...
  import type { OC_BillRetrievalList } from '@/api/oc/type/response';
  import {
    useAirConditioning,
    useAttendance,
    useOneCard,
